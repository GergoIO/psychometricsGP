    ...
    if (assessmentType %in% c("ADK")) {
      ...
      listOfItemReview <- list(
        itemReviewLowFac = toString(unique(sort(
          as.numeric(na.omit(itemAnalysisData$Item[itemAnalysisData[["Stage 5 Facility"]] < 0.2]))
        ))),

        # Negative Growth Items
        itemReviewNegGrowth = toString(unique(sort(
          as.numeric(na.omit(itemAnalysisData$Item[itemAnalysisData[["Stage 5 vs 2 Growth"]] < 0]))
        ))),

        # Negative PtBis Items
        itemReviewNegPtBis = toString(unique(sort(
          as.numeric(na.omit(itemAnalysisData$Item[itemAnalysisData[["Stage 5 PtBis"]] < 0]))
        ))),

        # Statistically Significant Negative PtBis Items
        itemReviewSigNegPtBis = toString(unique(sort(
          as.numeric(na.omit(itemAnalysisData$Item[itemAnalysisData[["Stage 5 Sig PtBis"]] < 0]))
        )))
      )
      ...
      listOfItemReview <- list(
        itemReviewLowFac = toString(unique(sort(
          as.numeric(na.omit(itemAnalysisData$Item[itemAnalysisData[["Stage 3 Facility"]] < 0.2]))
        ))),

        # Negative Growth Items
        itemReviewNegGrowth = toString(unique(sort(
          as.numeric(na.omit(itemAnalysisData$Item[itemAnalysisData[["Stage 3 vs 2 Growth"]] < 0]))
        ))),

        # Negative PtBis Items
        itemReviewNegPtBis = toString(unique(sort(
          as.numeric(na.omit(itemAnalysisData$Item[itemAnalysisData[["Stage 3 PtBis"]] < 0]))
        ))),

        # Statistically Significant Negative PtBis Items
        itemReviewSigNegPtBis = toString(unique(sort(
          as.numeric(na.omit(itemAnalysisData$Item[itemAnalysisData[["Stage 3 Sig PtBis"]] < 0]))
        )))
      )
      ...
      if (is.null(testInYear)) {
        ...
        if (testInYear %in% c(1, 2)) {
          ...
          listOfItemReview <- list(
            itemReviewLowFac = toString(unique(sort(
              as.numeric(na.omit(itemAnalysisData$Item[itemAnalysisData[["Stage 4 Facility"]] < 0.2]))
            ))),
            itemReviewNegPtBis = toString(unique(sort(
              as.numeric(na.omit(itemAnalysisData$Item[itemAnalysisData[["Stage 4 PtBis"]] < 0]))
            )))
          )
        }
      }
      ...
      listOfItemReview <- list(
        itemReviewLowFac = toString(unique(sort(
          as.numeric(na.omit(itemAnalysisData$Item[itemAnalysisData[["Stage 2 Facility"]] < 0.2]))
        ))),

        # Negative Growth Items
        itemReviewNegGrowth = toString(unique(sort(
          as.numeric(na.omit(itemAnalysisData$Item[itemAnalysisData[["Stage 2 vs 1 Growth"]] < 0]))
        ))),

        # Negative PtBis Items
        itemReviewNegPtBis = toString(unique(sort(
          as.numeric(na.omit(itemAnalysisData$Item[itemAnalysisData[["Stage 2 PtBis"]] < 0]))
        ))),

        # Statistically Significant Negative PtBis Items
        itemReviewSigNegPtBis = toString(unique(sort(
          as.numeric(na.omit(itemAnalysisData$Item[itemAnalysisData[["Stage 2 Sig PtBis"]] < 0]))
        )))
      )
      ...
      listOfItemReview <- list(
        itemReviewLowFac = toString(unique(sort(
          as.numeric(na.omit(itemAnalysisData$Item[itemAnalysisData[["Facility"]] < 0.2]))
        ))),

        # Negative PtBis Items
        itemReviewNegPtBis = toString(unique(sort(
          as.numeric(na.omit(itemAnalysisData$Item[itemAnalysisData[["PtBis"]] < 0]))
        ))),

        # Statistically Significant Negative PtBis Items
        itemReviewSigNegPtBis = toString(unique(sort(
          as.numeric(na.omit(itemAnalysisData$Item[itemAnalysisData[["Sig PtBis"]] < 0]))
        )))
      )
    }
...
#       unique(sort(as.numeric(c(
#         na.omit(itemAnalysisData$Item[itemAnalysisData[["Stage 5 Sig PtBis"]] < 0])
#       ))))
#     listOfItemReview$itemReviewSigNegPtBis <-
...
#     listOfItemReview$itemReviewSigNegPtBis <-
#       unique(sort(as.numeric(c(
#         na.omit(itemAnalysisData$Item[itemAnalysisData[["Stage 3 Sig PtBis"]] < 0])
#       ))))
#     listOfItemReview$itemReviewSigNegPtBis <-
#       toString(listOfItemReview$itemReviewSigNegPtBis, sep = ",")
#
...
#     listOfItemReview$itemReviewSigNegPtBis <-
#       unique(sort(as.numeric(c(
#         na.omit(itemAnalysisData$Item[itemAnalysisData[["Sig PtBis"]] < 0])
#       ))))
#     listOfItemReview$itemReviewSigNegPtBis <-
#       toString(listOfItemReview$itemReviewSigNegPtBis, sep = ",")
#
