      ...
      if (length(stages) > 1) {
        ...
        if ("Stage" %in% colnames(dfDemog) == FALSE) {
          ...
          for (i in stages) {
            ...
            valid_vars <- vars_anova[sapply(loopDemogData[, vars_anova], function(col)
              length(unique(col)) >= 2)]

            # Identify excluded variables
            excluded_vars <- setdiff(vars_anova, valid_vars)

            # Print message about excluded variables
            if (length(excluded_vars) > 0) {
              message(
                "The following variables were excluded due to having fewer than two factors: ",
                paste(excluded_vars, collapse = ", ")
              )
            } else {
              message("All variables in vars_anova have at least two factors.")
            }

            # Proceed with valid variables
            if (length(valid_vars) > 0) {
              aov <- aov(Score ~ ., data = na.omit(loopDemogData[, c("Score", valid_vars)]))
            } else {
        ...
        tabAov <-
          cbind(fnGPRound(tabAov[, 1:5], 2), (fnGPRound(tabAov[, 6], 3)))
        colnames(tabAov) <-
          c("Df", "Sum of Sq", "RSS", "AIC", "F-statistic", "P-value")
        tabAov <- rownames_to_column(tabAov, "Factor")
