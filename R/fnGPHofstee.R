...
fnGPHofstee <- function(x,
  ...
  nonPC <- sum(x > 100 |
                 x < 0)	# Identify (& remove) non-percentage scores
  if (nonPC > 0) {
    x <- x[x >= 0 & x <= 100]
    stop(
      paste0(
        ...
        " outside the range 0 to 100. Please check your data."
      )
    )
  }
  if (length(pars) != 4)
    stop("The argument 'pars' must be a vector of length 4")
  minCut <- pars[1]
  maxCut <- pars[2]
  minFail <- pars[3]
  maxFail <- pars[4]
  if (minCut > maxCut)
    stop("The minimum cut score must be less than the maximum cut score")
  if (minFail > maxFail)
    stop("The minimum failure rate must be less than the maximum failure rate")
  assign("HofsteeFail", FALSE, .GlobalEnv)
  # Create 100Ã—3 cumulative frequency table for the Hofstee calculations and graph
  ogiveData <-
    data.frame(Score = 0:100, Frequency = as.vector(table(
      cut(x, breaks = -1:100, include.lowest = TRUE)
    )))
  ogiveData$CumFreqPercent <-
    cumsum(ogiveData$Frequency) / max(cumsum(ogiveData$Frequency)) * 100
  s <- seq(length(ogiveData$Score) - 1)
  a <-
    psp(
      ogiveData$Score[s],
      ogiveData$CumFreqPercent[s],
      ogiveData$Score[s + 1],
      ogiveData$CumFreqPercent[s + 1],
  ...
  b <-
    psp(minCut,
        ...
        maxCut,
        minFail,
        window = owin(xrange = c(0, 100), yrange = c(0, 100)))	#Hofstee diagonal
  intersection <-
    data.frame(crossing.psp(a, b))#find out where the two lines cross and convert crossing point to data frame
  assign("HofsteePoint",
         c(x = intersection[1, 1], y = intersection[1, 2]),
         .GlobalEnv)
  if (is.na(HofsteePoint[1])) {
    ...
    if (failRate > maxFail) {
      ...
      assign(
        "HofsteeFailText",
        paste0(
          "Using the maximum failure rate of ",
          maxFail,
      ...
      assign(
        "HofsteeFailText",
        paste0(
          "Using the minimum failure rate of ",
          minFail,
  ...
  print(HofsteePoint)
  box <- data.frame(x = c(minCut, maxCut),
                    y = c(minFail, maxFail))
  hofsteeText1 <- "Hofstee cut score = "
  hofsteeText2 <-
    formatC(fnGPRound(HofsteePoint[1], cutDigits),
            digits = cutDigits,
            format = "f")
  redLineHeight <-
    ifelse(HofsteeFail, max(HofsteePoint[2] + 10, maxFail * 2), maxFail * 2)
  plot <- ggplot() +
    themeGP() +
    ...
    geom_vline(xintercept = minCut,
               size = 0.5,
               colour = "black") + #Line from Hofstee box
    geom_vline(xintercept = maxCut,
               size = 0.5,
               colour = "black") + #Line from Hofstee box
    geom_hline(yintercept = minFail,
               size = 0.5,
               colour = "black") + #Line from Hofstee box
    geom_hline(yintercept = maxFail,
               size = 0.5,
               colour = "black") + #Line from Hofstee box
    geom_segment(
      aes(
    ...
    geom_line(
      data = ogiveData,
      aes(x = Score, y = CumFreqPercent),
      size = 1.2,
      colour = "blue"
    ) +  #Ogive
    geom_point(aes(x = HofsteePoint[1], y = HofsteePoint[2]),
               size = 2,
               colour = "red") + #Hofstee point
    geom_segment(
      aes(
        x = HofsteePoint[1],
        y = min(100, redLineHeight),
        xend = HofsteePoint[1],
        yend = 0
      ),
      size = 1.2,
      colour = "red",
      linetype = "dashed"
    ) + #Red line at cut score
    annotate(
      geom = "text",
      x = HofsteePoint[1],
      y = min(98, redLineHeight + 1),
      label = hofsteeText1,
      fontface = 2,
      color = "red",
      vjust = "bottom",
  ...
  if (HofsteeFail) {
    ...
    cat(
      "         The variable 'HofsteeFail' has been set to TRUE and the following sentence has been saved as 'HofsteeFailText'.\n"
    )
    cat(HofsteeFailText)
    cat("\n")
  }
